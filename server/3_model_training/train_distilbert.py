...
def main():
    print('='*80)
    print('FINE-TUNING DistilBERT FOR EMOTION CLASSIFICATION')
    print('='*80)
    ...
    tokenizer = AutoTokenizer.from_pretrained('ml_models/distilbert')
    model = AutoModelForSequenceClassification.from_pretrained('ml_models/distilbert', num_labels=5, id2label={v:k for k,v in label_dict.items()}, label2id=label_dict)
    model.to(device)
    print('Model loaded\\n')
    ...
    visualizer = ComprehensiveVisualizer('DistilBERT Fine-tuned', results_dir, emotion_labels)
