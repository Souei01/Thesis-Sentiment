# Fine-tune mBERT for Emotion Classification
...
def main():
    print('='*80)
    print('FINE-TUNING mBERT FOR EMOTION CLASSIFICATION')
    print('='*80)
    ...
    print('Training will take ~50-60 minutes on CPU')
    print()
    ...
    print('Loading mBERT...')
    tokenizer = AutoTokenizer.from_pretrained('ml_models/mbert')
    model = AutoModelForSequenceClassification.from_pretrained('ml_models/mbert', num_labels=5, id2label={v:k for k,v in label_dict.items()}, label2id=label_dict)
    model.to(device)
    print('Model loaded\\n')
    ...
    visualizer = ComprehensiveVisualizer('mBERT Fine-tuned', results_dir, emotion_labels)
